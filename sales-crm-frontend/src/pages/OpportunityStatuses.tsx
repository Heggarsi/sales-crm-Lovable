...
import { TrendingUp, Plus, Search, MoreHorizontal, Edit, Trash2 } from "lucide-react";
...
const dummyData: OpportunityStatus[] = [
  { id: 1, statusName: "Open" },
  { id: 2, statusName: "In Progress" },
  { id: 3, statusName: "On Hold" },
  { id: 4, statusName: "Won" },
  { id: 5, statusName: "Lost" },
];

export default function OpportunityStatuses() {
  const [data, setData] = useState<OpportunityStatus[]>(dummyData);
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [isEditOpen, setIsEditOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<OpportunityStatus | null>(null);
  ...
  const handleEdit = (item: OpportunityStatus) => {
    setSelectedItem(item);
    setIsEditOpen(true);
  };

  const handleDelete = (item: OpportunityStatus) => {
    setSelectedItem(item);
  ...
  const FormFields = ({ item }: { item?: OpportunityStatus }) => (
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage opportunity status options</p>
          </div>
